...
SYNOPSIS
       ...
       slterm  [-f  regularfont]  [-fb  boldfont]  [-fi   italicfont]   [-fbi
       bolditalicfont] [-fw fontwidth] [-fh fontheight] [cmd ...]
...
TERMINAL
       ...
       slterm starts either the env variable SHELL,  /bin/sh,  or  a  command
       supplied with slterm [-e] command options.

       All options are forwarded to command without changes.

       The  already  set  environmental variables are kept, further variables
   ...
   Colors
       ...
       BG(background), BGL(ight background).   Example:  $LGREEN,  D:  $DRED,
       $LD: $LDCYAN, BG: $BGBLUE, BGLCYAN.

       These  Colors  cannot  be combined: ORANGE, ORANGERED, BROWN, BGBROWN,
       PURPLE, GRAY, MINT, TURQUOISE

       Other  text  attributes  are:  $BOLD,  $FAINT,  $CURSIVE,  $UNDERLINE,
       $BLINK, $REVERSE, $STRIKETHROUGH, $DOUBLEUNDERLINE.

       The  text  attributes can be combined, with some special combinations:
       BLINK and REVERSE blinks by reversing colors.  STRIKETHROUGH  and  UN-
       DERLINE get a double underline.

       The  default foreground and background color and attributes can be re-
       set with $NORMAL.
...
INSTALL
       ...
       2. install  the terminal info file: (for curses) slterm -I | tic -sx -
          ( the netbsd version of tic, the terminal info compiler,   is  sup-
          plied  as  source in tools/tic, and should be compatible with other
          curses versions )
       ...
       Interestingly, the statically linked binary seems  to  use  even  less
       memory than the shared binary.
   ...
   CURSES
          ...
          |  tic  -sx  -.  The key combination Ctrl+Shift+Win+ALT+I will dump
          the terminal info to the terminal as well, and can be used, to  in-
          stall  the  terminal  info within a remote shell. ( type tic -sx -,
          hit Ctrl+Shift+Win+Alt+I, and Ctrl+D )
...
OPTIONS
       ...
       -f font
              ...
              'Liberation         Mono:Bold:pixelsize=13:antialias=true:auto-
              hint=true' the parameters are described in the fontconfig docu-
              mentation, an overview is supplied in doc/fontconfig.txt

       -fb boldfont -fi italicfont -fI bolditalicfont
              Set  bold/italic/bolditalic  fonts.   Supply '0' to disable the
              according font and to display  the  text  attributes  by  color
              changes only
       ...
       -o iofile
              ...
              recording slterm sessions. A value of "-" means  standard  out-
              put.
       ...
       -l line
              ...
              (pseudo-)serial device (e.g. /dev/ttyS0  on  Linux  for  serial
              port  0).  When this flag is given remaining arguments are used
              as flags for stty(1). By default slterm initializes the  serial
              line  to  8  bits, no parity, 1 stop bit and a 38400 baud rate.
              The speed is set by appending it as last argument (e.g. 'slterm
              -l  /dev/ttyS0  115200').   Arguments  before  the last one are
              stty(1) flags. If you want to set odd parity on 115200 baud use
              for  example  'slterm  -l /dev/ttyS0 parenb parodd 115200'. Set
              the number of bits by using for example 'slterm  -l  /dev/ttyS0
              cs7 115200'. See stty(1) for more arguments and cases.
       ...
       -e command [ arguments ... ]
              slterm  executes  command instead of the shell. If this is used
              it must be the last option on the command line, as in  xterm  /
              rxvt.  This  option is only intended for compatibility, and all
       ...
       -X     lock all memory pages into memory, prevent  swapping.   Secrets
              could  be  revealed, also years later, if the memory is swapped
              to disk. Worse, with flash disks also erasing the contents will
              not necessarily erase the written cells.  This option locks all
...
AUTHORS
       ...
       Ori  Bernstein,  Matthias  Schoth,  Laslo  Hunhold,  Paride  Legovini,
       Lorenzo  Bracco,  Kamil  Kleban,  Avi Halachmi, Jacob Prosser, Augusto
       Born de Oliveira, Kai Hendry, Laslo  Hunhold,  Matthew  Parnell,  Doug
       Whiteley, Aleksandrs Stier, Devin J. Pohly, Sai Praneeth Reddy
